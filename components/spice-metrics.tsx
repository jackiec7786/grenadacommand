  ...
  return (
    <div className="bg-surface border border-border rounded-md p-5 border-t-2" style={{ borderTopColor: 'var(--accent)' }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-[9px] font-mono text-muted-foreground tracking-[0.1em]">
          spiceclassifieds.com
        </div>
      </div>
            ...
            <div key={key}>
              <div className="flex items-end justify-between mb-1.5">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    className="bg-dim border border-border font-mono text-[15px] font-bold px-2 py-1 rounded-sm w-20 text-right focus:outline-none focus:border-primary"
                    style={{ color }}
                    value={val || ''}
                    placeholder="0"
                    min={0}
      ...
      <div className="mt-4 pt-4 border-t border-border">
        {spice.listings < 50 && (
          <div className="text-[11px] font-mono text-warn">
            ⚠ Fix payment bug before promoting listings
          </div>
        )}
        {spice.listings >= 50 && spice.listings < 200 && (
          <div className="text-[11px] font-mono text-muted-foreground">
            Growing — reach 200 before running paid ads
          </div>
        )}
        {spice.listings >= 200 && spice.listings < 500 && (
          <div className="text-[11px] font-mono text-accent2">
            On track — push WhatsApp and Business Pro outreach
          </div>
        )}
        {spice.listings >= 500 && (
          <div className="text-[11px] font-mono text-primary">
            ✓ Listing target hit — activate paid tiers and event categories
          </div>
        )}
      </div>
    </div>

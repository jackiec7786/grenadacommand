  ...
  return (
    <div className="bg-surface border border-border rounded-md p-5 animate-fade-slide-in" style={{ borderTopWidth: 2, borderTopColor: config.cssColor, animationDelay: '0.1s' }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setEditMode(e => !e)} title="Edit tasks" className="cursor-pointer text-muted-foreground hover:text-text transition-all">
      ...
      <div className="flex gap-1.5 mb-2 flex-wrap">
        {[1, 2, 3, 4].map(p => {
          ...
          return (
            <button key={p} onClick={() => onPhaseChange(p)} className="text-[10px] font-mono px-2.5 py-1 rounded-sm border cursor-pointer transition-all tracking-[0.1em]"
      ...
      <div className="flex flex-col gap-2">
        {phaseTasks.map((task, i) => {
          ...
          return (
            <div key={i} className={`flex items-start gap-3 p-2.5 px-3 rounded border cursor-pointer transition-all bg-surface2 border-border group`}>
              {editMode ? (
                <>
                  {editIdx === i ? (
                  ...
                  <button onClick={() => removeTask(i)} className="text-muted-foreground hover:text-danger font-mono text-[13px] px-1 cursor-pointer shrink-0">×</button>
                </>
              ) : (
                <>
                  ...
                  <span onClick={() => onTaskToggle(phaseKey, i)} className={`text-[13px] leading-relaxed flex-1 ${isDone ? 'text-muted-foreground line-through' : 'text-text'}`}>
                    {task.label}
                  </span>
      ...
      {editMode && (
        <div className="flex gap-2 mt-3 pt-3 border-t border-border">
          <input className={inp} placeholder="New task label..." value={newLabel} onChange={e => setNewLabel(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addTask() }} />
          <button onClick={addTask} disabled={!newLabel.trim() || saving} className="font-mono text-[10px] uppercase px-3 py-1.5 rounded-sm cursor-pointer disabled:opacity-40" style={{ background: 'var(--accent)', color: 'var(--bg)' }}>
            {saving ? '...' : 'Add'}
          </button>
        </div>
      )}
